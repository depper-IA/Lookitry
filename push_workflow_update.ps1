...
# THE FIXED PROMPT - Much more explicit about JSON-only, in English for better model compliance
...
# Create the update payload - need to match n8n API format
$updatePayload = @{
    name = $workflow.name
    nodes = @($workflow.activeVersion.nodes)
...
Write-Host "=== UPDATE PAYLOAD SIZE ==="
Write-Host ($updatePayload.Length)
Write-Host ""
