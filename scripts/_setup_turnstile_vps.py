...
_, out, _ = ssh.exec_command("cat /root/Lookitry/docker-compose.frontend.yml")
...
if "TURNSTILE_SITE_KEY" not in content:
    ...
    stdin, _, _ = ssh.exec_command("cat > /root/Lookitry/docker-compose.frontend.yml")
    stdin.write(content)
...
# Verificar
_, out, _ = ssh.exec_command("grep TURNSTILE /root/Lookitry/docker-compose.frontend.yml")
print("docker-compose:", out.read().decode().strip())

# 2. Agregar al backend .env del VPS
_, out, _ = ssh.exec_command("grep TURNSTILE /root/Lookitry/backend/.env")
...
if "TURNSTILE" not in existing:
    ...
    stdin, _, _ = ssh.exec_command("cat >> /root/Lookitry/backend/.env")
