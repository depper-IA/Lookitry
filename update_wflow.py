...
path = r'c:\Users\Usuario\Mostrador_wilkiedevs\templates-webs\_wf_main_raw.json'
with open(path, 'r', encoding='utf-8-sig') as f:
    data = json.load(f)

for n in data.get('nodes', []):
    if n['name'] == 'Validar Input':
        n['parameters']['jsCode'] = """const item = $input.item.json;
...
return { json: { brand_id: body.brand_id, product_id: body.product_id, selfie_url: body.selfie_url, product_image_url: body.product_image_url, prompt: body.prompt, timestamp, timestamp_safe } };"""
    elif n['name'] == 'Preparar Prompt Gemini':
        n['parameters']['jsCode'] = """const data = $('Validar Input').item.json;
return { json: { brand_id: data.brand_id, product_id: data.product_id, product_image_url: data.product_image_url, timestamp: data.timestamp, timestamp_safe: data.timestamp_safe, full_prompt: data.prompt, selfie_url: data.selfie_url } };"""
    elif n['name'] == 'Generar con Gemini':
        n['parameters']['jsonBody'] = """={{ JSON.stringify({ "model": "google/gemini-2.5-flash-image", "modalities": ["image", "text"], "messages": [{ "role": "user", "content": [ { "type": "text", "text": $json.full_prompt }, { "type": "image_url", "image_url": { "url": $json.selfie_url } }, { "type": "image_url", "image_url": { "url": $json.product_image_url } } ] }], "max_tokens": 1024, "temperature": 0.3 }) }}"""
    elif n['name'] == 'Extraer Imagen Base64':
        n['parameters']['jsCode'] = """const r = $input.item.json;
...
data['nodes'] = [n for n in data.get('nodes', []) if n['name'] != 'Subir Selfie Temporal']

if 'Validar Input' in data.get('connections', {}):
    data['connections']['Validar Input']['main'] = [[{"node":"Preparar Prompt Gemini","type":"main","index":0}]]
if 'Subir Selfie Temporal' in data.get('connections', {}):
    del data['connections']['Subir Selfie Temporal']
