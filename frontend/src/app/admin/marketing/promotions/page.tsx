...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ backgroundColor: checked ? '#FF5C3A' : '#333' }}
      role="switch"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Nombre interno</label>
          <input
            value={form.name}
            onChange={e => set('name', e.target.value)}
            placeholder="Ej: Oferta Black Friday"
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white placeholder-[#444] focus:outline-none focus:border-[#FF5C3A] transition-colors"
          />
        </div>
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Tipo</label>
          <select
            value={form.type}
            onChange={e => set('type', e.target.value as PromoType)}
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-[#FF5C3A] transition-colors"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Inicio (opcional)</label>
          <input
            type="datetime-local"
            value={form.starts_at ? form.starts_at.slice(0, 16) : ''}
            onChange={e => set('starts_at', e.target.value ? new Date(e.target.value).toISOString() : undefined)}
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-[#FF5C3A] transition-colors"
          />
        </div>
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Fin (opcional)</label>
          <input
            type="datetime-local"
            value={form.ends_at ? form.ends_at.slice(0, 16) : ''}
            onChange={e => set('ends_at', e.target.value ? new Date(e.target.value).toISOString() : undefined)}
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-[#FF5C3A] transition-colors"
          />
        </div>
      </div>
      ...
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 space-y-3">
        <p className="text-[11px] font-semibold text-[#666] uppercase tracking-wide">Configuración — {TYPE_LABELS[form.type]}</p>
        ...
        {form.type === 'plan_override' && (
          <>
            <div>
              <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Plan</label>
              <select
                value={String(cfg.plan ?? 'BASIC')}
                onChange={e => setConfig('plan', e.target.value)}
                className="w-full bg-[#141414] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-[#FF5C3A]"
              >
        ...
        {form.type === 'coupon' && (
          <p className="text-[12px] text-[#555]">Los cupones se gestionan en la sección de Cupones más abajo.</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[13px] text-[#aaa]">Activa esta promoción inmediatamente</span>
      </div>
      ...
      <div className="flex gap-2 pt-2">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-semibold text-white transition-all disabled:opacity-50"
        ...
        <button
          onClick={onCancel}
          className="px-4 py-2 rounded-lg text-[13px] text-[#888] hover:text-white border border-[#2a2a2a] hover:border-[#444] transition-colors"
        >
          Cancelar
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full bg-[#141414] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white placeholder-[#444] focus:outline-none focus:border-[#FF5C3A] transition-colors"
      />
    </div>
  );
}
...
function CouponForm({
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Código</label>
          <input
            value={form.code}
            onChange={e => set('code', e.target.value.toUpperCase().replace(/\s/g, ''))}
            placeholder="BLACKFRIDAY20"
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white font-mono placeholder-[#444] focus:outline-none focus:border-[#FF5C3A] transition-colors"
          />
        </div>
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Tipo de descuento</label>
          <select
            value={form.discount_type}
            onChange={e => set('discount_type', e.target.value)}
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-[#FF5C3A]"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">
        ...
        <div>
          <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Usos máximos (vacío = ilimitado)</label>
          <input
            type="number"
            value={form.max_uses ?? ''}
            onChange={e => set('max_uses', e.target.value ? Number(e.target.value) : undefined)}
            placeholder="Ilimitado"
            className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white placeholder-[#444] focus:outline-none focus:border-[#FF5C3A]"
          />
        </div>
      </div>
      <div>
        <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Fecha de expiración (opcional)</label>
        <input
          type="datetime-local"
          value={form.expires_at ? form.expires_at.slice(0, 16) : ''}
          onChange={e => set('expires_at', e.target.value ? new Date(e.target.value).toISOString() : undefined)}
          className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-[13px] text-white focus:outline-none focus:border-[#FF5C3A]"
        />
      </div>
      <div>
        <label className="block text-[11px] font-semibold text-[#666] uppercase tracking-wide mb-1">Planes aplicables (vacío = todos)</label>
        <div className="flex gap-2">
          {['BASIC', 'PRO', 'LANDING'].map(p => {
            ...
            return (
              <button
                key={p}
                onClick={() => set('plan_ids', active ? form.plan_ids.filter(x => x !== p) : [...form.plan_ids, p])}
                className="px-3 py-1.5 rounded-lg text-[12px] font-medium border transition-all"
                style={{
                  borderColor: active ? '#FF5C3A' : '#2a2a2a',
                  backgroundColor: active ? 'rgba(255,92,58,0.1)' : '#1a1a1a',
                  color: active ? '#FF5C3A' : '#666',
                }}
              >
                {p}
              </button>
      ...
      <div className="flex items-center gap-3">
        <Toggle checked={form.active} onChange={v => set('active', v)} />
        <span className="text-[13px] text-[#aaa]">Cupón activo</span>
      </div>
      <div className="flex gap-2 pt-1">
        ...
        <button onClick={onCancel} className="px-4 py-2 rounded-lg text-[13px] text-[#888] hover:text-white border border-[#2a2a2a] hover:border-[#444] transition-colors">
          Cancelar
        </button>
      </div>
    </div>
...
export default function PromotionsPage() {
  ...
  const loadCoupons = useCallback(async () => {
    const token = getToken();
    // Leer cupones directamente desde Supabase via service key no disponible en cliente
    // Usamos la API pública con service key en el backend
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(255,92,58,0.15)' }}>
            <IconMegaphone />
          </div>
          <div>
      ...
      {error && (
        <div className="bg-[#1f0f0f] border border-[#5a1a1a] text-[#ff6b6b] text-[12px] rounded-lg px-4 py-3 flex items-center justify-between">
          {error}
      ...
      <div className="flex gap-1 p-1 rounded-xl w-fit" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
        {(['promos', 'coupons'] as const).map(t => (
          <button
            ...
            style={{
              backgroundColor: tab === t ? '#FF5C3A' : 'transparent',
      ...
      {tab === 'promos' && (
        <div className="space-y-4">
          {/* Formulario inline */}
          {showPromoForm && (
            <div className="rounded-xl p-5 space-y-1" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
              <p className="font-syne font-semibold text-[15px] mb-4" style={{ color: 'var(--text-primary)' }}>
                {editingPromo ? 'Editar promoción' : 'Nueva promoción'}
              </p>
              <PromoForm
                initial={editingPromo ? { type: editingPromo.type, name: editingPromo.name, config: editingPromo.config, active: editingPromo.active, starts_at: editingPromo.starts_at, ends_at: editingPromo.ends_at } : EMPTY_PROMO}
          ...
          {/* Lista */}
          {loading ? (
            <div className="flex justify-center py-12"><IconSpinner /></div>
          ) : promos.length === 0 ? (
            <div className="text-center py-16 rounded-xl" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
              <IconTag />
              <p className="text-[13px] mt-3" style={{ color: 'var(--text-secondary)' }}>No hay promociones. Crea la primera.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                    <th className="text-left px-5 py-3 font-semibold text-[11px] uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Nombre</th>
                    <th className="text-left px-5 py-3 font-semibold text-[11px] uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Tipo</th>
                    <th className="text-left px-5 py-3 font-semibold text-[11px] uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Vigencia</th>
                    <th className="text-center px-5 py-3 font-semibold text-[11px] uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Activa</th>
                    <th className="px-5 py-3" />
                  </tr>
                </thead>
                <tbody className="divide-y" style={{ borderColor: 'var(--border-color)' }}>
                  {promos.map(p => (
                    <tr key={p.id} className="transition-colors" style={{ '--hover-bg': 'var(--bg-hover)' } as React.CSSProperties}
                      onMouseEnter={e => (e.currentTarget as HTMLElement).style.backgroundColor = 'var(--bg-hover)'}
                      onMouseLeave={e => (e.currentTarget as HTMLElement).style.backgroundColor = 'transparent'}
                    >
                      <td className="px-5 py-3.5 font-medium" style={{ color: 'var(--text-primary)' }}>{p.name}</td>
                      <td className="px-5 py-3.5">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium text-white"
        ...
        <div className="space-y-4">
          ...
          {showCouponForm && (
            <div className="rounded-xl p-5" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
              ...
              <CouponForm
                initial={editingCoupon ? { code: editingCoupon.code, discount_type: editingCoupon.discount_type, discount_value: editingCoupon.discount_value, max_uses: editingCoupon.max_uses, expires_at: editingCoupon.expires_at, plan_ids: editingCoupon.plan_ids, active: editingCoupon.active } : EMPTY_COUPON}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-xl" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
              <p className="text-[13px] mt-3" style={{ color: 'var(--text-secondary)' }}>No hay cupones. Crea el primero.</p>
            </div>
          ) : (
            <div className="rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
              <table className="w-full text-[13px]">
                  ...
                  {coupons.map(c => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-block w-2 h-2 rounded-full ${c.active ? 'bg-emerald-500' : 'bg-[#333]'}`} />
                      </td>
                      <td className="px-4 py-3">
