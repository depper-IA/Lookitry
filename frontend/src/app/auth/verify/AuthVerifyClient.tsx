...
export function AuthVerifyClient({ token }: { token: string | null }) {
  ...
  useEffect(() => {
    if (!token) {
      setStatus('error');
      setMessage('Token de verificacion no encontrado.');
      return;
    }
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/verify-email?token=${token}`, {
      ...
      .then(data => {
        if (data.error) {
          setStatus('error');
          setMessage(data.message || 'El enlace es invalido o ya expiro.');
        } else {
          setStatus('success');
