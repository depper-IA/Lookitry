...
export function ErrorBanner({ error, isService = false }: { error: string | null; isService?: boolean }) {
  ...
  if (isService || error === 'SERVICE_CREDITS_EXHAUSTED') {
    return (
      <div className="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-2xl flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-gray-600">La prueba virtual está temporalmente no disponible</p>
          <p className="text-xs text-gray-400 mt-0.5">El servicio de generación se quedó sin capacidad temporalmente. Intenta de nuevo en unos minutos.</p>
        </div>
      </div>
    );
  }
