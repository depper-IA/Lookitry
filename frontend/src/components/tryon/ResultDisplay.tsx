    ...
    <>
      <div className="max-w-md mx-auto">
        ...
        {selfiePreview && (
          <div className="grid grid-cols-2 gap-3 mt-4 mb-2">
            <div className="relative">
              ...
              <div className="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-0.5 rounded-full uppercase font-bold tracking-wider">Antes</div>
            </div>
            <div className="relative group cursor-pointer" onClick={() => setLightboxOpen(true)}>
              <ResultImage
                ...
                brandName={brandName}
              />
              <div className="absolute bottom-2 left-2 text-white text-xs px-2 py-0.5 rounded-full pointer-events-none uppercase font-bold tracking-wider" style={{ backgroundColor: `${primaryColor}cc` }}>Después</div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            {downloading ? '...' : (
              <>
                ...
                Descargar imagen con mi producto
              </>
            )}
          </button>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleShareFacebook} className="flex-1 py-3 rounded-2xl border border-gray-100 flex items-center justify-center gap-2 hover:bg-gray-50 transition-all">
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4" onClick={() => setLightboxOpen(false)}>
          <img src={imageUrl} alt={productName} className="max-w-full max-h-[90vh] object-contain" />
          <button className="absolute top-6 right-6 text-white"><svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M6 18L18 6M6 6l12 12" strokeWidth={2}/></svg></button>
        </div>
      )}
    </>
  );
