...
export function UpgradeModal({ isOpen, onClose }: UpgradeModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div
        className="relative w-full max-w-lg overflow-hidden rounded-3xl bg-dark border border-white/10 shadow-2xl shadow-black/50"
      ...
      >
        {/* Premium glow effect */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 h-48 w-80 bg-gradient-to-b from-accent/30 to-transparent" aria-hidden="true" />

        {/* Close button - fixed position, outside scroll container */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 rounded-full p-2 text-white/40 transition-colors hover:bg-white/10 hover:text-white min-h-[44px] min-w-[44px] flex items-center justify-center"
          aria-label="Cerrar"
        >
          <X size={20} />
        </button>
        ...
        <div className="overflow-y-auto max-h-[90vh]">
          <div className="relative px-8 pt-10 pb-8 text-center">
            {/* Icon */}
            <div className="relative mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-accent/30 to-accent/5 border border-accent/30">
            ...
            <Link
              ...
              onClick={onClose}
              className="group relative mb-4 flex w-full items-center justify-center gap-3 overflow-hidden rounded-2xl bg-accent px-8 py-5 font-bold text-white shadow-2xl shadow-accent/30 transition-all hover:scale-[1.02] hover:bg-accent/90 hover:shadow-accent/40 active:scale-[0.98]"
            >
              <Gift size={22} />
            ...
            <p className="mt-6 text-xs text-white/30">
              Desde {basicPrice}/mes · Sin compromiso · Cancela cuando quieras
            </p>
          </div>
        </div>
      </div>
