...
export default function GoogleSignInButton({ onSuccess, onError, mode = 'login' }: GoogleSignInButtonProps) {
  ...
  const handleGoogleResponse = async (response: any) => {
    ...
    try {
      ...
      if (!apiRes.ok) {
        onError?.(data.message || 'Error al iniciar sesión con Google');
        return;
      }
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-6 py-3 text-sm font-semibold text-white transition-all hover:bg-white/10"
    >
