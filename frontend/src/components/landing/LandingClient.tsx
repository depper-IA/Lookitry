...
export default function LandingClient({ pricing }: { pricing: PricingConfig }) {
  ...
  const basicFeatures = [
    ...
    `${(pricing?.basic?.generaciones_mensuales ?? 400).toLocaleString()} generaciones/mes`,
  ...
  const proFeatures = [
    ...
    `${(pricing?.pro?.generaciones_mensuales ?? 1200).toLocaleString()} generaciones/mes`,
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f5f2ee]">
      ...
      <section className="bg-[#0a0a0a] py-16 md:py-20 px-6 md:px-8" aria-labelledby="pricing-heading">
        <div className="max-w-[700px] mx-auto">
          <p className="sr-only">
            Lookitry ofrece dos planes mensuales: Plan Básico cuesta {formatPrice(basicPrice)} con {pricing?.basic?.productos_max} productos y {pricing?.basic?.generaciones_mensuales} generaciones al mes.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 pt-4">
            ...
            <div className="bg-[#141414] border border-[#2a2a2a] hover:border-[#FF5C3A]/60 transition-colors duration-200 rounded-xl p-6 md:p-7 flex flex-col h-full">
              ...
              <div className="mb-0.5">
                {basicOrig > basicPrice && <span className="text-[14px] text-[#666] line-through mr-2">{formatPrice(basicOrig)}</span>}
                <span className="font-syne font-extrabold text-[30px] text-white tracking-tight">
                  {formatPrice(basicPrice)}
                </span>
                <span className="text-[13px] font-normal text-[#555] ml-1">/ mes</span>
              </div>
            ...
            <div className="bg-[#141414] border border-[#FF5C3A] rounded-xl p-6 md:p-7 relative flex flex-col h-full">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#FF5C3A] text-white text-[10px] font-semibold tracking-widest uppercase px-3 py-1 rounded-full whitespace-nowrap" aria-label="Plan más popular">
                Más popular
              </div>
              <div className="font-syne font-bold text-lg text-white mb-1">Pro</div>
              <div className="mb-0.5">
                {proOrig > proPrice && <span className="text-[14px] text-[#666] line-through mr-2">{formatPrice(proOrig)}</span>}
                <span className="font-syne font-extrabold text-[30px] text-white tracking-tight">
            ...
            {STEPS.map((s, i) => (
              <div key={s.n} className="bg-white border border-[#e8e4df] rounded-2xl overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                <div className="relative w-full aspect-[3/4] overflow-hidden bg-[#f0ece8]">
                  <Image src={s.img} alt={s.alt} fill className={`object-cover ${s.pos}`} sizes="(max-width: 768px) 100vw, 33vw" priority={i === 0} />
                  <div className="absolute top-3 left-3">
                    <span className="font-syne inline-block text-white font-extrabold text-sm px-2.5 py-1 rounded-lg leading-none bg-[#FF5C3A]">{s.n}</span>
                  </div>
                </div>
                <div className="p-5">
                ...
                {MINI_LANDING_FEATURES.map(f => (
                  <div key={f.title} className="bg-white border border-[#e8e4df] hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 rounded-2xl p-5 flex gap-3">
                    <div className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center" style={{ background: 'rgba(255,92,58,0.1)' }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FF5C3A" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d={f.icon} /></svg>
                    </div>
                    <div>
      ...
      <section className="bg-[#0a0a0a] py-16 md:py-20 px-6 md:px-8 text-center relative overflow-hidden">
        <div className="absolute pointer-events-none" style={{ width: '500px', height: '300px', background: 'radial-gradient(ellipse, rgba(255,92,58,0.06) 0%, transparent 70%)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
        <div className="relative z-10 max-w-xl mx-auto">
          <h2 className="font-syne font-extrabold text-[34px] md:text-[38px] text-white tracking-tight mb-3">Empieza hoy sin riesgos</h2>
          <p className="text-[15px] text-[#aaa] mb-8">7 días gratis con el plan Trial. Plan Básico y Pro con pago directo y seguro.</p>
          <div className="flex gap-3 justify-center flex-wrap">
            <Link href="/register" className="bg-[#FF5C3A] hover:bg-[#e84d2c] text-white text-[15px] font-medium px-8 py-3.5 rounded-xl transition-all duration-200 hover:-translate-y-0.5 active:scale-95">Crear cuenta gratis</Link>
            <CtaProButton />
          </div>
        </div>
      </section>
