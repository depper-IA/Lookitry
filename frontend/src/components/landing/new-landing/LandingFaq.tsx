            ...
            return (
              <div key={idx} className="border-b border-white/5 last:border-0 font-dm-sans">
                ...
                >
                  ...
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center shrink-0 transition-all duration-500 ${open ? 'bg-[#FF5C3A] border-[#FF5C3A] rotate-180' : 'border-white/10 text-white/20'
                    }`}>
                    <ChevronDown size={18} />
                  </div>
                </button>
                {open && (
                  <div className="pb-8 text-white/40 text-sm leading-relaxed max-w-3xl animate-in fade-in slide-in-from-top-2 duration-300">
                    {item.a}
                  </div>
                )}
              </div>
