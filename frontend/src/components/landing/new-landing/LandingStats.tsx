...
export default function LandingStats() {
  ...
  return (
    <section className="bg-[#0a0a0a] py-12 sm:py-16 md:py-20 px-4 sm:px-6" aria-label="Estadísticas">
      <div className="max-w-7xl mx-auto px-0 sm:px-6 md:px-12 flex flex-wrap justify-center md:justify-between items-center gap-8 sm:gap-10 md:gap-8">
        {stats.map((stat, i) => (
          <div key={i} className="flex items-center gap-3 sm:gap-4 md:gap-5">
            ...
            <div>
              <div className="font-jakarta text-2xl sm:text-3xl font-bold text-white mb-0.5">{stat.val}</div>
              <div className="font-dm-sans text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-white/60">{stat.label}</div>
            </div>
          </div>
        ))}
      </div>
