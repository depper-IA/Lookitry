...
async function sendExpirationNotifications(days: number): Promise<number> {
  ...
  try {
    ...
    console.log(`\nð§ Enviando notificaciones para marcas que vencen en ${days} días...`);
    ...
    for (const brand of brandsExpiringInExactDays) {
      ...
      try {
        ...
        console.error(`   â Error al enviar notificación a ${brand.email}:`, error);
    ...
    console.log(`   â ${sentCount} notificación(es) enviada(s) para ${days} días`);

    return sentCount;

  } catch (error) {

    console.error(`   â Error al enviar notificaciones para ${days} días:`, error);
...
async function sendTrialEndingNotifications(): Promise<number> {
  ...
  try {
    ...
    console.log('\nð§ Enviando notificaciones de trial por vencer...');
    ...
    if (!trialBrands || trialBrands.length === 0) {
      ...
      console.log('   â¹ï¸  No hay trials por vencer');
      ...
      if (endDate >= targetStart && endDate < targetEnd) {
        ...
        if (daysRemaining >= 2 && daysRemaining <= 3) {
          ...
          try {
            ...
            console.error(`   â Error al enviar trial ending a ${brand.email}:`, err);
    ...
    console.log(`   â ${sentCount} notificación(es) de trial por vencer enviada(s)`);

    return sentCount;

  } catch (error) {

    console.error('   â Error enviando notificaciones de trial por vencer:', error);
...
async function sendTrialNudgeSetupNotifications(): Promise<number> {
  ...
  try {
    ...
    console.log('\nð§ Enviando notificaciones de nudge de setup a trials...');
    ...
    if (!trialBrands || trialBrands.length === 0) {
      ...
      console.log('   â¹ï¸  No hay trials para nudge de setup');
        ...
        if (daysRemaining >= 2 && daysRemaining <= 3) {
          ...
          if (!hasProducts || !hasLanding) {
            ...
            try {
              ...
              console.error(`   â Error al enviar nudge setup a ${brand.email}:`, err);
    ...
    console.log(`   â ${sentCount} notificación(es) de nudge setup enviada(s)`);

    return sentCount;

  } catch (error) {

    console.error('   â Error enviando notificaciones de nudge setup:', error);
...
async function sendSuspensionNotifications(): Promise<number> {
  ...
  try {
    ...
    console.log('\nð§ Enviando notificaciones de suspensión...');
    ...
    if (!suspendedBrands || suspendedBrands.length === 0) {
      ...
      console.log('   â¹ï¸  No hay marcas recién suspendidas para notificar');
    ...
    console.log(`   â ${sentCount} notificación(es) de suspensión enviada(s)`);

    return sentCount;

  } catch (error) {

    console.error('   â Error al enviar notificaciones de suspensión:', error);
...
export async function runDailySubscriptionCheck() {
  ...
  try {
    ...
    console.log('ð Actualizando estados de suscripciones...');
    ...
    console.log('\nâ Actualización de estados completada\n');

    console.log('ð Resumen de actualizaciones:');
    ...
    console.log('â Verificación completada exitosamente\n');

    console.log('ð Resumen de acciones realizadas:');
    ...
    console.log(`\n   ð§ Total de notificaciones enviadas: ${totalNotifications}`);
    ...
    if (result.expired > 0) {
      ...
      console.log(`\nâ ï¸  ${result.expired} suscripción(es) vencieron hoy`);
    ...
    if (result.expiringSoon > 0) {
      ...
      console.log(`â° ${result.expiringSoon} suscripción(es) vencen en menos de 7 días`);
    ...
    if (result.suspended > 0) {
      ...
      console.log(`ð« ${result.suspended} marca(s) fueron suspendidas por falta de pago`);
    ...
    if (result.expired === 0 && result.expiringSoon === 0 && result.suspended === 0 && totalNotifications === 0) {
      ...
      console.log('\nâ¨ No hay suscripciones que requieran actualización ni notificaciones que enviar');
    ...
    console.error('\nâ Error durante la verificación de suscripciones:');
