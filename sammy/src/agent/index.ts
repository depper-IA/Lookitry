...
export class Agent {
  ...
  async run(userMessage: string, conversationId = 'default'): Promise<string> {
    ...
    const markdownPrompt = this.getSystemPrompt();

    // If the markdown prompt is empty/missing, use the legacy fallback prompt.
    const fallbackPrompt = `You are Sammy, a helpful personal AI assistant connected to the user's project repository. Reply in Spanish by default unless the user asks for another language. You can use tools when they improve accuracy. Be concise, practical, and safe.`;

    const systemPrompt = `${markdownPrompt || fallbackPrompt}
